...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { code, productId, clientId, subtotalCents } = validateCouponSchema.parse(body);
    ...
    if (coupon.assignedTo && clientId) {
      try {
        const assignedTo = JSON.parse(coupon.assignedTo);
        if (assignedTo !== "all" && Array.isArray(assignedTo) && !assignedTo.includes(clientId)) {
          return apiSuccess({
            valid: false,
