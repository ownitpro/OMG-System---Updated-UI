...
export async function POST(req: Request) {
  try {
    ...
    const leadData = {
      ...
      desiredGoal: body.desiredGoal || null,
    ...
    console.log("Contractors Lead:", leadData);

    return NextResponse.json({ 
      success: true, 
      message: "Lead submitted successfully",
      leadId: `ct-${Date.now()}` // Temporary ID
    });

  } catch (error) {
    console.error("Error processing contractors lead:", error);
    return NextResponse.json(
