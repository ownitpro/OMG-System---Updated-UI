...
const CUSTOM_PROJECTS = [
  ...
  },
  {
    ...
    estimatedCompletion: "Feb 28, 2025",
    color: "#A855F7",
  },
];
...
export default function CustomSolutionsPage() {
  ...
  const activeProjects = CUSTOM_PROJECTS.length;
  const completedProjects = COMPLETED_PROJECTS.length;

  return (
    <PortalShellV2 role="client" title="Custom Solutions" nav={nav} upgradeHref="/products/plans" lockedCount={0}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
          ...
          >
            ...
            <div className="mt-1 text-2xl font-bold text-[#A855F7]">6 weeks</div>
          </div>
          <div
            className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-5"
          ...
          >
            ...
            <div className="mt-1 text-2xl font-bold text-[#F59E0B]">2</div>
          </div>
        </div>
            ...
            {CUSTOM_PROJECTS.map((project) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  >
                    <WrenchScrewdriverIcon className="w-5 h-5" style={{ color: project.color }} />
                  </div>
                  <div>
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ backgroundColor: `${project.color}20` }}
                  >
                    <CheckCircleIcon className="w-4 h-4" style={{ color: project.color }} />
                  </div>
                  <div>
                    <div className="font-medium text-white">{project.name}</div>
                    <div className="text-xs text-white/50">Completed {project.completedDate}</div>
                  </div>
                </div>
                <button className="rounded-lg bg-white/5 border border-white/10 px-3 py-1.5 text-xs text-white hover:bg-white/10 transition-colors flex items-center gap-1">
